...
function TodoListItem({ todo, setTodos }) {
    ...
    const onDelete = async () => {
        try {
            // 삭제 요청
            await axios.delete(`https://todoapp-spring-brook-5982-little-grass-565-silent-shape-3149.fly.dev/kim/todos/${id}`);
    ...
    const onToggle = async () => {
        try {
            // PATCH request
            await axios.patch(
                `https://todoapp-spring-brook-5982-little-grass-565-silent-shape-3149.fly.dev/kim/todos/${id}`,
                { completed: !completed },
                {
                    headers: {
